...
def detect_keypoint(dataset):
    ...
    for group in dataset.keys():
        for i, img in enumerate(dataset[group]):
            kp, desc = orb.detectAndCompute(img[1], None)
            desc = util.int_to_binary(desc)
...
def join_with_centroid(df_centroid, descriptors):
    ...
    descriptors_only = np.array(descriptors_.iloc[:, range(256)], dtype='uint8')
    data_only = descriptors_.iloc[:, range(256, 263)]    

    descriptors_only = pd.DataFrame(util.binary_to_int(descriptors_only))
    descriptors_ = pd.concat([descriptors_only, data_only], axis=1)
...
def main(args):
    ...
    desc_type = ClusterModel.Descriptor.ORB
    ...
    cm_name = f'{dataset_name}_orb_agglo_{maxsize}.cm'
    cm.save(cm_name)
